...
app.get('/api/history/:id',requireUser,async(req,res)=>{
  const {data,error}=await supabaseAdmin.from('calculation_history').select('id,title,mode,input_json,result_json,created_at').eq('id',req.params.id).eq('user_id',req.user.id).maybeSingle();
  if(error)return sendDbError(res,error,'Không tải được chi tiết lịch sử.');
  if(!data)return res.status(404).json({code:'HISTORY_NOT_FOUND',error:'Không tìm thấy bản lịch sử này.'});
