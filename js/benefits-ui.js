...
function calculatorState() {
  return window.PensionCalculatorState || { getPeriods: () => [], getEntryMode: () => 'manual', getImportJobId: () => null, getPerson: () => ({}) };
}
function refreshOneTimePeriodSummary() {
  const periods = calculatorState().getPeriods?.() || [];
  const el = $('oneTimePeriodSummary');
...
function eligibilityItems(data) {
  return data?.details?.eligibilityErrors || data?.details?.errors || [];
}
function renderCalculationError(host, err) {
  if (!host) return;
  const items = eligibilityItems(err.data);
  if (err.status === 422) {
    host.innerHTML = `<div class="alert error"><strong>Chưa đủ điều kiện hưởng theo dữ liệu đã nhập.</strong><p>Lượt tính không bị trừ.</p>${items.length ? `<ul>${items.map(x => `<li>${esc(x)}</li>`).join('')}</ul>` : ''}</div>`;
  } else {
    host.innerHTML = `<div class="alert error"><strong>Chưa thể tính:</strong> ${esc(err.message)}${err.status && err.status !== 402 ? '<p>Lượt tính chưa bị trừ vì chưa tạo được kết quả hợp lệ.</p>' : ''}</div>`;
  }
...
async function calculateCommercial(body) {
  ...
  if (!result?.ok) throw Object.assign(new Error((result?.errors || ['Dữ liệu chưa đủ để tính.']).join(' ')), { status: 400, data: { details: result } });
  if (result?.eligible === false) throw Object.assign(new Error('Chưa đủ điều kiện hưởng theo dữ liệu đã nhập. Lượt tính không bị trừ.'), { status: 422, data: { details: result, charged: false } });
...
async function saveBenefit(type, button, status) {
  ...
  try {
    ...
    if (status) status.textContent = e.message || 'Không lưu được.';
    if (/hết lượt|mua thêm/i.test(e.message || '')) window.PensionAccount?.openPlans?.();
...
function renderUnemployment(data, request) {
  ...
  host.innerHTML = `<div class="result-heading"><div><span class="step">KẾT QUẢ</span><h2>Ước tính trợ cấp thất nghiệp</h2></div></div><div class="result-grid">${metric('Mức hưởng/tháng', money.format(r.monthlyBenefit), true)}${metric('Số tháng hưởng', `${r.durationMonths} tháng`)}${metric('Tổng ước tính', money.format(r.estimatedTotal))}${metric('Bình quân 06 tháng', money.format(r.averageSix))}${metric('Trần/tháng', money.format(r.ceiling))}${metric('Thời gian BHTN chưa sử dụng', `${r.eligibleContributionMonths} tháng`)}${metric(`Đóng trong ${r.lookbackWindow} tháng trước nghỉ`, `${r.lookbackContributionMonths} tháng`)}</div>${alertList('Lưu ý', r.warnings)}${alertList('Điều kiện đã áp dụng', r.notes, 'info')}${saveControls('unemployment')}`;
...
function renderMaternity(data, request) {
  ...
  host.innerHTML = `<div class="result-heading"><div><span class="step">KẾT QUẢ</span><h2>${esc(MATERNITY_CASE_LABELS[r.caseType] || 'Chế độ thai sản')}</h2></div></div><div class="result-grid">${metric('Tổng mức hưởng ước tính', money.format(r.estimatedTotal || 0), true)}${r.monthlyBenefit != null ? metric('Mức trợ cấp tháng', money.format(r.monthlyBenefit)) : ''}${r.durationLabel ? metric('Thời gian tính hưởng', r.durationLabel) : ''}${r.lumpSum != null ? metric('Trợ cấp một lần', money.format(r.lumpSum || 0)) : ''}${r.dailyBenefit != null ? metric('Mức hưởng/ngày', money.format(r.dailyBenefit)) : ''}${r.referenceLevel ? metric('Mức tham chiếu', money.format(r.referenceLevel)) : ''}</div>${r.eligibilityRule ? `<div class="explain"><h3>Điều kiện đã áp dụng</h3><p>${esc(r.eligibilityRule)}</p></div>` : ''}${alertList('Lưu ý', r.warnings)}${alertList('Cách tính/Phạm vi', r.notes, 'info')}${saveControls('maternity')}`;
  wireSave(host, 'maternity'); host.scrollIntoView({ behavior: 'smooth', block: 'start' });
}

const ONE_TIME_HELP = {
  retirement_age: 'Kiểm tra đủ tuổi nghỉ hưu theo ngày sinh/giới tính và tổng thời gian đóng phải chưa đủ 15 năm.',
  emigration: 'Trường hợp ra nước ngoài để định cư không áp dụng giới hạn dưới 20 năm, nhưng phải đã chấm dứt tham gia BHXH.',
  serious_condition: 'Áp dụng cho bệnh nặng hoặc mức suy giảm khả năng lao động/khuyết tật đặc biệt nặng thuộc diện luật định.',
  pre2025_after12months: 'Chỉ áp dụng khi có thời gian đóng trước 01/07/2025, đã ngừng tham gia đủ 12 tháng và tổng thời gian đóng chưa đủ 20 năm.',
  other: 'Hãy chọn một trường hợp cụ thể ở trên để công cụ có thể kiểm tra điều kiện trước khi tính.'
};
function syncOneTimeReason() {
  const reason = $('oneTimeReason')?.value || '';
  if ($('oneTimeReasonHelp')) $('oneTimeReasonHelp').textContent = ONE_TIME_HELP[reason] || '';
  if ($('oneTimeTwelveMonthRow')) $('oneTimeTwelveMonthRow').hidden = reason !== 'pre2025_after12months';
  if ($('oneTimeSeriousConditionRow')) $('oneTimeSeriousConditionRow').hidden = reason !== 'serious_condition';
}

function syncMaternityForm() {
  const scheme = $('maternityScheme')?.value || 'compulsory';
  const caseSel = $('maternityCase');
  if (!caseSel) return;
...
function init() {
  ...
  $('oneTimeReason')?.addEventListener('change', syncOneTimeReason); syncOneTimeReason();
  $('oneTimeForm')?.addEventListener('submit', async e => {
    e.preventDefault();
    const state = calculatorState(); const periods = state.getPeriods?.() || []; const host = $('oneTimeResult');
    if (!periods.length) { host.innerHTML = '<div class="alert error">Chưa có quá trình đóng BHXH. Hãy nhập hoặc đọc hồ sơ ở tab Lương hưu trước.</div>'; return; }
    const request = {
      benefitType: 'one_time', mode: state.getEntryMode?.() || 'manual', importJobId: state.getImportJobId?.() || null,
      periods, settlementMonth: parseMonth($('oneTimeSettlementMonth').value), eligibilityReason: $('oneTimeReason').value,
      actualPaidVnd: Number($('oneTimeActualPaid').value || 0), person: state.getPerson?.() || {},
      stoppedParticipation: $('oneTimeStoppedParticipation').checked, stopped12Months: $('oneTimeStopped12Months').checked,
      seriousConditionConfirmed: $('oneTimeSeriousConditionConfirmed').checked
    };
    const b = e.submitter || e.currentTarget.querySelector('button[type="submit"]'); if (b) { b.disabled = true; b.textContent = 'Đang kiểm tra…'; }
    try { renderOneTime(await calculateCommercial(request), request); }
    catch (err) { renderCalculationError(host, err); if (err.status === 402) window.PensionAccount?.openPlans?.(); if (err.status === 422) try { await window.PensionAccount?.refreshMe?.(); } catch {} }
    finally { if (b) { b.disabled = false; b.textContent = 'Tính BHXH một lần'; } }
  });

  $('unemploymentForm')?.addEventListener('submit', async e => {
    e.preventDefault(); const host = $('unemploymentResult');
    const input = {
      eligibleContributionMonths: Number($('unemploymentMonths').value || 0),
      lookbackContributionMonths: Number($('unemploymentLookbackMonths').value || 0),
      contractGroup: $('unemploymentContractGroup').value,
      wasContributingAtTermination: $('unemploymentWasContributing').checked,
      lawfulTermination: $('unemploymentLawfulTermination').checked,
      filedWithin3Months: $('unemploymentFiledWithin3Months').checked,
      noExclusionAfter10Days: $('unemploymentNoExclusion').checked,
      lastContributionMonth: parseMonth($('unemploymentLastMonth').value), region: $('unemploymentRegion').value,
      lastSixSalaries: [...document.querySelectorAll('.unemployment-salary')].map(x => Number(x.value || 0))
    };
    const request = { benefitType: 'unemployment', mode: 'manual', input };
    const b = e.submitter || e.currentTarget.querySelector('button[type="submit"]'); if (b) { b.disabled = true; b.textContent = 'Đang kiểm tra…'; }
    try { renderUnemployment(await calculateCommercial(request), request); }
    catch (err) { renderCalculationError(host, err); if (err.status === 402) window.PensionAccount?.openPlans?.(); if (err.status === 422) try { await window.PensionAccount?.refreshMe?.(); } catch {} }
  ...
  $('maternityForm')?.addEventListener('submit', async e => {
    ...
    const request = { benefitType: 'maternity', mode: 'manual', input };
    const b = e.submitter || e.currentTarget.querySelector('button[type="submit"]'); if (b) { b.disabled = true; b.textContent = 'Đang kiểm tra…'; }
    try { renderMaternity(await calculateCommercial(request), request); }
    catch (err) { renderCalculationError(host, err); if (err.status === 402) window.PensionAccount?.openPlans?.(); if (err.status === 422) try { await window.PensionAccount?.refreshMe?.(); } catch {} }
    finally { if (b) { b.disabled = false; b.textContent = 'Tính chế độ thai sản'; } }
  });
}
