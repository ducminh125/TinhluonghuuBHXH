-- VN Pension Calculator v3.1.2 - Supabase schema
-- IMPORTANT: Run from Supabase Dashboard > SQL Editor.
-- If Studio is currently impersonating the `authenticated` role, RESET ROLE returns
-- the session to its original Dashboard role before the migration checks privileges.
...
BEGIN
  IF current_user <> 'postgres' THEN
    RAISE EXCEPTION 'MIGRATION_MUST_RUN_AS_POSTGRES: current_user=%, session_user=%', current_user, session_user
      USING HINT = 'In Supabase SQL Editor, open the Role dropdown and select postgres/default, then run again.';
  END IF;
END $$;
...
-- Admin note: do not create a separate admin signup. Add the admin email to Vercel ADMIN_EMAILS.
-- On the next authenticated request, the server promotes that profile to role='admin'.
